...
```{ojs}
//| label: carga-datos-mapa
//| echo: false

topo    = fetch("data/colombia_moderate.topojson").then(r => r.json())
pob_raw = fetch("data/poblacion_mun_2026.json").then(r => r.json())
...
######## Figura 1 Mapa interactivo del departamento de Córdoba y sus municipios. Fuente: elaboración propia a partir de IGAC / GADM v4.1
...
//| echo: false
datos_dep_raw = fetch("data/piramide_dep.json").then(r => r.json())
```
...
######## Figura 3 Participación por actividad económica en el departamento de Córdoba, 2022. Fuente: UPRA (2024) y DANE, pg. 8.
...
######## Figura 5 Mapa geológico de Córdoba, pagina 74. FUENTE: Plan Departamental de Gestión del Riesgo de Desastres, Gobernación de Córdoba (2022), pg. 82.
